...
### II.2. Impact measure with flows: 166
...
为了使得the number of commuters is conserved，需要对参数进行微调，即基于工作岗位减少前的$A_i$，对$A_i$进行重新计算分配。最后使用与III.2中相同方法对flow进行计算。
## IV.2
...
$$
使用上述公式，我们可以通过控制$\mu$的大小来调控交通成本的变化。假设交通成本增加，那么对$\mu$分别取值$\mu _1=1.1, \mu _2=1.3$. 最终计算得到两类flow。
