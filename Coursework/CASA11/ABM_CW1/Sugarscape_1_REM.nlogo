...
globals[
  ...
  end_population ;; the population of turtles in the end
  ;;rate_survive ;; the rate of survival turtles

  ;;max_initial_sugar ;; the max initial suger of each agents
  mean_ini_sugar;;======================
  ;;avg_sugar ;; the average wealth/sugar
  ;;dev_sugar ;; the standard deviation sugar          code: standard-deviation list

  mean_ini_vision;;======================
  mean_ini_metabolism;;======================

  ;;rate_increasable ;; the rate of people who can earn more money

  suger_list ;; the list of turtles' sugar
]
;; ------------------------------The Code I insert --------------------------------
...
to setup
  ...
  set initial_population random-in-range 1 max_initial_population

  ;; ------------------------------The Code I insert --------------------------------

  create-turtles initial_population [ turtle-setup ]
  setup-patches

  ;; ------------------------------The Code I insert --------------------------------

  set mean_ini_sugar mean [sugar] of turtles
  set mean_ini_vision mean [vision] of turtles
  set mean_ini_metabolism mean [metabolism] of turtles
...
to turtle-setup ;; turtle procedure
  ...
  set sugar random-in-range 1 max_initial_sugar
  set metabolism random-in-range 1 max_metabolism
  set vision random-in-range 1 max_vision
...
to go
  ...
  if all? turtles [max-psugar >= metabolism]
  [
    set end_population count turtles
    ;;set avg_sugar mean [sugar] of turtles
    ;;set dev_sugar standard-deviation [sugar] of turtles

    ;;set rate_survive end_population / initial_population
    ;;set rate_increasable count turtles with [max-psugar > metabolism] / end_population
    stop
  ]
...
to-report sugar_list
 report [sugar] of turtles
end

to-report rate_survive  ;;======================
  report end_population / initial_population
end
...
to-report dev_sugar  ;;======================
  report standard-deviation [sugar] of turtles
end

to-report rate_increasable  ;;======================
  report count turtles with [max-psugar > metabolism] / end_population
end
  ;; ------------------------------The Code I insert --------------------------------
...
@#$#@#$#@
<experiments>
  <experiment name="test1" repetitions="10000" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>initial_population</metric>
    <metric>mean_ini_sugar</metric>
    <metric>mean_ini_vision</metric>
    <metric>mean_ini_metabolism</metric>
    <metric>rate_survive</metric>
    <metric>avg_sugar</metric>
    <metric>dev_sugar</metric>
    <metric>rate_increasable</metric>
    <enumeratedValueSet variable="max_initial_sugar">
      <value value="30"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max_initial_population">
      <value value="2499"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max_vision">
      <value value="10"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max_metabolism">
      <value value="4"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
