globals[
  ;;population ;; the population of turtles in T
  ini_pop
  suger_avg
  vision_avg
  metabolism_avg
  ...
  pop_pre ;; the popluation of turtles in T-1
  pop_T;; the population of turtles in T
  flag
...
to setup
  clear-all
  set ini_pop initial-population

  create-turtles ini_pop [ turtle-setup ]
  setup-patches


  ;; ------------------------------The Code I insert --------------------------------
  set ini_vision_list [vision] of turtles
  set ini_metabolism_list [metabolism] of turtles
  set pop_T initial-population
  set flag 0
  ;; ------------------------------The Code I insert --------------------------------
...
to turtle-setup ;; turtle procedure
  ...
  set sugar ini_sugar
  set metabolism ini_metabolism
  set vision ini_vision
...
to go
  ...
  set pop_pre pop_T
  set pop_T count turtles

  ifelse pop_T = pop_pre
  [
    if flag > 25
    [
      set pop_T count turtles
      set suger_avg mean [sugar] of turtles
      set vision_avg mean [vision] of turtles
      set metabolism_avg mean [metabolism] of turtles
      set survive_rate pop_T / ini_pop
      set num_growable count turtles with [max-psugar > metabolism]
      set rate_growable num_growable / pop_T
      stop
    ]
...
to-report rate_survive  ;;======================
  report pop_T / initial-population
end
...
to-report rate_increasable  ;;======================
  report count turtles with [max-psugar > metabolism] / pop_T
end
  ;; ------------------------------The Code I insert --------------------------------
...
@#$#@#$#@
<experiments>
  <experiment name="experiment_test" repetitions="5" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    ...
    <steppedValueSet variable="initial-population" first="0" step="400" last="2400"/>
    <steppedValueSet variable="ini_metabolism" first="1" step="1" last="4"/>
    <steppedValueSet variable="ini_vision" first="1" step="1" last="10"/>
    <steppedValueSet variable="ini_sugar" first="5" step="5" last="30"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
